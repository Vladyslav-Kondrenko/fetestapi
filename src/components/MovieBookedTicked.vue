...
import type { BookPlaceData } from '@/api/api'
import { Button } from '@/components/ui/button';
import { TicketCheck } from 'lucide-vue-next';

defineProps<{
  bookedPlace: BookPlaceData,
	bookedMoovieName: string
}>();
      ...
      <DialogHeader>
        ...
        <div class="cardWrap" v-if="bookedPlace">
          <div class="card cardLeft">
            ...
            <div class="title">
              <h2> {{bookedMoovieName}} </h2>
              <span>movie</span>
            </div>
           
            <div class="seat">
              <h2>{{bookedPlace.seat}}</h2>
              <span>seat</span>
            </div>
            <div class="time">
              <h2>{{bookedPlace.daytime}}</h2>
              <span>time</span>
            </div>
